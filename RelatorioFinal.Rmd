...
A próxima análise refere em descobrir em quais meses do ano a uma maior incidência de acidentes. Ela é obtida através do seguinte script
...
Em seguida, convertemos a variável month para o formato do tipo data.

```{r}
mes_acidente$month <- as.Date(as.yearmon(mes_acidente$month))
```

Assim podemos gerar o seguinte gráfico
...
Essa tabela pode ser melhor visualizada através do uso de bibliotecas, ficando desse jeito
...
#Gerar o gráfico.
barplot(aux, las=2, col= rainbow(10), cex.names= 0.6)
```

A frequência de palavras mais usadas estão entre elas "atenção", "falta", "condução", "condutor", "desobidiência", "normas". Podemos levantar a hipótese de que os acidentes ou a grande maioria deles são falhas humanas. Já que as palavras mais frequentes referem-se a requerer atenção dos motoristas 
